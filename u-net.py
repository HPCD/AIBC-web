...
"""
@author:abner
@file:Segmentation_training.py
@ datetime:2020/10/15 16:58
@software: PyCharm
...
tensorboard = TensorBoard(log_dir="bank_card")
...
X_NUM = 72  # your traning data number
train_path = "D:/abner/project/pyproject/labelme-master/examples/instance_segmentation/data_dataset_bankcard3/"

pathX = train_path+"512/images/"    #change your file path
pathY = train_path+"512/labels/"   #change your file path
X_NUM = len(os.listdir(pathX))
step_epoch = int(X_NUM/BATCH_SIZE)
print(("x ",X_NUM))
...
def generator(pathX, pathY,BATCH_SIZE):
    while 1:
        ...
        for i in range(BATCH_SIZE):
            ...
            # print("img path ",pathX + X_train_files[index])
            #
...
history = model.fit_generator(generator(pathX, pathY,BATCH_SIZE),
                              steps_per_epoch=step_epoch, nb_epoch=EPOCH,callbacks=callback_lists)
end_time = datetime.datetime.now().strftime('%Y-%m-%d  %H:%M:%S')

 #save your training model
model.save(r'bank_card0105.h5')

#save your loss data
# mse = np.array((history.history['loss']))
# np.save(r'./id_ocr2.npy', mse)
