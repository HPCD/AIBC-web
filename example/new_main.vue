...
export default {
  ...
  methods: {
    // 初始化请求数据，如标签，坐标信息
    initLabelData() {
      var hurl = "http://127.0.0.1:9999/src/labels";
      // alert("d");
      let _this = this;
      //后台请求数据
      $.ajax({
    ...
    saveLastImageInfo(lastImageInfo){
      ...
      var save_imgInfo_url = "http://127.0.0.1:9999/src/img_info";
      //异步请求
      $.ajax({
        url: save_imgInfo_url,
        type: "put",
    ...
    setImgCanvas(url) {
      ...
      console.log("simg ",_this.singleImg);
      //提交上一张图片信息保存
      this.saveLastImageInfo(_this.singleImg),
      // 首先清空之前画布
      _this.canvas.clear();
